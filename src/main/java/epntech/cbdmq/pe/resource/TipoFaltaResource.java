...
import epntech.cbdmq.pe.servicio.impl.TipoFaltaServiceImpl;
...
public class TipoFaltaResource {
    ...
    public ResponseEntity<?> guardar(@RequestBody TipoFalta obj) throws DataException {
    	obj.setNombreFalta(obj.getNombreFalta().toUpperCase());
    	return new ResponseEntity<>(objServices.save(obj), HttpStatus.OK);
    }

    @GetMapping("/listar")
    public List<TipoFalta> listar() {
        return objServices.getAll();
    }

    @GetMapping("/{id}")
    public ResponseEntity<TipoFalta> obtenerDatosPorId(@PathVariable("id") Integer codigo) {
        return objServices.getById(codigo).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> actualizarDatos(@PathVariable("id") Integer codigo, @RequestBody TipoFalta obj) throws DataException {
        return (ResponseEntity<TipoFalta>) objServices.getById(codigo).map(datosGuardados -> {
            datosGuardados.setNombreFalta(obj.getNombreFalta().toUpperCase());
            datosGuardados.setEstado(obj.getEstado());
            TipoFalta datosActualizados = null;
			try {
