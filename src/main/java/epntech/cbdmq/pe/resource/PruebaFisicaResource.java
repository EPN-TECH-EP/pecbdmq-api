...
@RestController
@RequestMapping("/pruebafisica")
public class PruebaFisicaResource {

	@Autowired
    private PruebaFisicaServiceImpl objServices;
	
	 @PostMapping("/crear")
	    @ResponseStatus(HttpStatus.CREATED)
	    public ResponseEntity<?> guardar(@RequestBody PruebaFisica obj) throws DataException {
	    	return new ResponseEntity<>(objServices.save(obj), HttpStatus.OK);
	    }
	 
	 @GetMapping("/listar")
	    public List<PruebaFisica> listar() {
	        return objServices.getAll();
	    }
	 
	 @GetMapping("/{id}")
	    public ResponseEntity<PruebaFisica> obtenerDatosPorId(@PathVariable("id") Integer codigo) {
