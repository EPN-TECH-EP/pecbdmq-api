...
public class ParametrizaPruebaResumenResource {
	...
	@Autowired
	private ParametrizaPruebaResumenServiceImpl objService;
	
	@PostMapping("/crear")
	@ResponseStatus(HttpStatus.CREATED)
	public ResponseEntity<?> guardar(@RequestBody ParametrizaPruebaResumen obj) throws DataException{
		return new ResponseEntity<>(objService.save(obj), HttpStatus.OK);
	}
	
	@GetMapping("/{id}")
	public ResponseEntity<ParametrizaPruebaResumen> obtenerPorId(@PathVariable("id") int codigo) {
		return objService.getById(codigo).map(ResponseEntity::ok)
				.orElseGet(() -> ResponseEntity.notFound().build());
	}
	
	@GetMapping("/listar")
	public List<ParametrizaPruebaResumen> listar() {
	...
	public ResponseEntity<ParametrizaPruebaResumen> actualizarDatos(@PathVariable("id") int codigo, @RequestBody ParametrizaPruebaResumen obj) throws DataException{
		return (ResponseEntity<ParametrizaPruebaResumen>) objService.getById(codigo).map(datosGuardados -> {
			...
			ParametrizaPruebaResumen datosActualizados = null;
			try {
