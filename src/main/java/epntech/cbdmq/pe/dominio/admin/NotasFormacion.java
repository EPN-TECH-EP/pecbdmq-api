...
@NamedNativeQuery(name = "EstudianteDatos.getNotasEstudiantesMateria",
		query = "select gnf.cod_nota_formacion,ge.cod_estudiante,ge.codigo_unico_estudiante, gdp.cedula ,gdp.nombre || '  ' ||  gdp.apellido as \"nombre\", gnf.nota_materia , gnf.nota_disciplina , gnf.nota_supletorio,mp.cod_paralelo, p.nombre_paralelo from {h-schema}gen_nota_formacion gnf\n" +
...
@NamedNativeQuery(name = "NotaMateriaByEstudianteFormacion.get",
		query = "select gnf.cod_nota_formacion, gmp.cod_materia_paralelo as cod_materia_curso,gm.nombre_materia, gnf.nota_materia , gnf.nota_disciplina , gnf.nota_supletorio, gimp.cod_instructor, gdp.nombre || ' ' || gdp.apellido as nombre_completo_instructor, ge.estado as nombre_periodo_formacion from cbdmq.gen_nota_formacion gnf \n" +
				...
				"and gti.nombre_tipo_instructor = :tipoInstructor \n" +
				"and ge.estado <> 'BAJA' \n" +
				"and mp.cod_periodo_academico =  :codPA \n" +
				"and gnf.cod_nota_formacion = :notaFormacion",resultSetMapping = "NotaMateriaByEstudianteAndNotaFormacion"
)
@SqlResultSetMapping(name = "NotaMateriaByEstudianteAndNotaFormacion", classes =
@ConstructorResult(
		targetClass = NotaMateriaByEstudiante.class,
		columns = {
				@ColumnResult(name = "cod_nota_formacion", type= Integer.class),
