...
public class InscripcionForServiceImpl implements InscripcionForService {
    ...
    @Override
    public InscripcionResult insertarInscripcionConDocumentos(InscripcionFor inscripcion, List<MultipartFile> docsInscripcion) throws IOException, ArchivoMuyGrandeExcepcion, MessagingException, ParseException, DataException {
        ...
        if (!periodoAcademicoRepository.getActive()) throw new DataException(PA_INACTIVO);
    ...
    public String savePostulante(Postulante obj, String proceso, String claveOriginal, String hashPassword, String correo) throws DataException, MessagingException {
        ...
        if (isPasswordMatches(claveOriginal, hashPassword).equals(true)) {
            ...
            if (postulante != null) {
                emailService.sendInscripcionForEmail(correo, obj.getIdPostulante());
                return postulante.getIdPostulante();
            } else throw new DataException(ERROR_REGISTRO + " [id postulante]");
        } else throw new DataException(PIN_INCORRECTO);
    }
    ...
    public InscripcionFor reenvioPin(InscripcionFor obj) throws DataException, MessagingException, IOException {
        String code = "";

        code = getRandomCode();

        if (this.findByCorreoPersonal(obj)) throw new DataException(MensajesConst.CORREO_YA_EXISTE);
    ...
    public Boolean findByCedula(String cedula, Convocatoria convocatoria) {
        ...
        if (inscripcion.isPresent()) {
            Optional<Postulante> postulante = this.repoPostulante.findByCodDatoPersonalAndCodPeriodoAcademico(inscripcion.get().getCodDatoPersonal(), convocatoria.getCodPeriodoAcademico());
    ...
    public Boolean findByCorreoPersonal(String correo, Convocatoria convocatoria) {
        ...
        if (!listaInscripcion.isEmpty()) {
            ...
            for (InscripcionFor inscripcionFor : listaInscripcion) {
                ...
                Optional<Postulante> postulante = this.repoPostulante.findByCodDatoPersonalAndCodPeriodoAcademico(inscripcionFor.getCodDatoPersonal(), convocatoria.getCodPeriodoAcademico());
    ...
    public Boolean findByCorreoPersonal(InscripcionFor inscripcion) {
        ...
        Optional<Postulante> postulanteOpt = this.repoPostulante.findByCodDatoPersonalAndCodPeriodoAcademico(inscripcion.getCodDatoPersonal(), convocatoria.getCodPeriodoAcademico());

        if (postulanteOpt.isPresent()) {

            // obtiene dato personal existente
            Optional<InscripcionFor> inscripcionExistenteOpt = this.repo1.findById(postulanteOpt.get().getCodDatoPersonal());

            if (inscripcionExistenteOpt.isPresent()) {
                // compara el correo registrado antes con el enviado para el reintento. Si es
                // igual, sale
                if (inscripcionExistenteOpt.get().getCorreoPersonal().compareToIgnoreCase(inscripcion.getCorreoPersonal()) == 0) {
                    retval = false;
                } else {
